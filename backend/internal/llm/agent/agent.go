...
func (a *agent) Run(ctx context.Context, sessionID string, content string, attachments ...message.Attachment) (<-chan AgentEvent, error) {
	...
	go func() {
		...
		for _, attachment := range attachments {
			// Convert documents (docx, pdf, txt, etc.) to text instead of sending as binary
			if message.IsDocumentType(attachment.MimeType) {
				var text string
				var err error
				if attachment.FilePath != "" {
					text, err = message.ExtractDocumentText(attachment.FilePath, attachment.MimeType)
				} else {
					text, err = message.ExtractDocumentTextFromBytes(attachment.Content, attachment.FileName, attachment.MimeType)
				}
				if err != nil {
					logging.Warn("Failed to extract document text, skipping attachment",
						"file", attachment.FileName, "error", err)
					attachmentParts = append(attachmentParts, message.TextContent{
						Text: fmt.Sprintf("[Could not extract text from %s: %v]", attachment.FileName, err),
					})
					continue
				}
				header := fmt.Sprintf("--- Attached document: %s ---\n", attachment.FileName)
				attachmentParts = append(attachmentParts, message.TextContent{Text: header + text})
			} else {
