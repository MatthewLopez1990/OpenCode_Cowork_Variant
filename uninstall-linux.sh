...
echo "  - $APP_NAME binary and desktop entry"
...
if [ -f "$SHELL_PROFILE" ] && grep -qE "COWORK_API_KEY|OPENROUTER_API_KEY" "$SHELL_PROFILE" 2>/dev/null; then
...
# Step 7: Clean PATH
if [ -f "$SHELL_PROFILE" ] && grep -q "\.opencode" "$SHELL_PROFILE" 2>/dev/null; then
    grep -v "\.opencode" "$SHELL_PROFILE" > "${SHELL_PROFILE}.tmp"
    mv "${SHELL_PROFILE}.tmp" "$SHELL_PROFILE"
