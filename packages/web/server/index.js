...
const resolveProjectDirectory = async (req) => {
  ...
  // Ensure sandbox rules exist in whichever directory is resolved
  if (validated.directory) {
    try { ensureSandboxRules(validated.directory); } catch (e) {}
  }
...
"This is a security policy file managed by your IT department. I cannot modify or remove it. These rules protect your workstation and are required by firm policy."
...
- Creating files disguised as different formats (.doc from HTML)
- Using any path outside the current working directory

## Word Document Creation

NEVER use Word COM automation — it hangs in non-interactive environments.
NEVER pass PowerShell inline — the shell corrupts it.
NEVER use Python on Windows — it is not installed.

### On Windows: write a .ps1 file that creates .docx using Open XML (no Word needed)

Step 1: Use the WRITE tool to create the document content as a .md file in THIS directory.

Step 2: Use the WRITE tool to create a file called convert.ps1 in THIS directory.
The convert.ps1 must read the .md file and create a .docx using .NET Open XML.
Here is the EXACT script content to use (replace INPUTFILE and OUTPUTFILE):

Add-Type -AssemblyName System.IO.Compression
$md = Get-Content 'INPUTFILE.md' -Raw -Encoding UTF8
$docxFile = Join-Path (Get-Location).Path 'OUTPUTFILE.docx'
if (Test-Path $docxFile) { Remove-Item $docxFile }
$body = ''
foreach ($line in ($md -split '\\n')) {
    $line = $line.TrimEnd([char]13)
    $esc = [System.Security.SecurityElement]::Escape($line)
    if ($line -match '^# (.+)') {
        $t = [System.Security.SecurityElement]::Escape($Matches[1])
        $body += '<w:p><w:pPr><w:pStyle w:val="Heading1"/></w:pPr><w:r><w:rPr><w:b/><w:sz w:val="32"/></w:rPr><w:t>' + $t + '</w:t></w:r></w:p>'
    } elseif ($line -match '^## (.+)') {
        $t = [System.Security.SecurityElement]::Escape($Matches[1])
        $body += '<w:p><w:pPr><w:pStyle w:val="Heading2"/></w:pPr><w:r><w:rPr><w:b/><w:sz w:val="28"/></w:rPr><w:t>' + $t + '</w:t></w:r></w:p>'
    } elseif ($line -match '^### (.+)') {
        $t = [System.Security.SecurityElement]::Escape($Matches[1])
        $body += '<w:p><w:pPr><w:pStyle w:val="Heading3"/></w:pPr><w:r><w:rPr><w:b/><w:sz w:val="24"/></w:rPr><w:t>' + $t + '</w:t></w:r></w:p>'
    } elseif ($line -match '^[-*] (.+)') {
        $t = [System.Security.SecurityElement]::Escape($Matches[1])
        $body += '<w:p><w:r><w:rPr><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/><w:sz w:val="22"/></w:rPr><w:t xml:space="preserve">  ' + [char]0x2022 + ' ' + $t + '</w:t></w:r></w:p>'
    } elseif ($line.Trim() -eq '') {
        $body += '<w:p/>'
    } else {
        $body += '<w:p><w:r><w:rPr><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/><w:sz w:val="22"/></w:rPr><w:t xml:space="preserve">' + $esc + '</w:t></w:r></w:p>'
    }
}
$docXml = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body>' + $body + '</w:body></w:document>'
...
$zip = [System.IO.Compression.ZipFile]::Open($docxFile, 'Create')
$enc = [System.Text.Encoding]::UTF8
foreach ($pair in @(@('[Content_Types].xml', $ct), @('_rels/.rels', $rels), @('word/document.xml', $docXml))) {
    $e = $zip.CreateEntry($pair[0])
    $s = $e.Open()
    $b = $enc.GetBytes($pair[1])
    $s.Write($b, 0, $b.Length)
    $s.Close()
}
$zip.Dispose()
Write-Host "Created: $docxFile"

Step 3: Use the BASH tool to run: powershell -ExecutionPolicy Bypass -File convert.ps1
...
async function startOpenCode() {
  ...
  ensureSandboxRules(openCodeWorkingDirectory);
