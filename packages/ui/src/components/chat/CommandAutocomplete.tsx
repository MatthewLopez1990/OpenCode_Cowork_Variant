...
export const CommandAutocomplete = React.forwardRef<CommandAutocompleteHandle, CommandAutocompleteProps>(({
      ...
      try {
        ...
        const builtInCommands: CommandInfo[] = [
          ...(hasSession && !hasMessagesInCurrentSession
            ? [{ name: 'init', description: 'Initialize project configuration', isBuiltIn: true }]
            : []
          ),
  ...
  const getCommandIcon = (command: CommandInfo) => {
    ...
    switch (command.name) {
      ...
      case 'contract':
      case 'letter':
        return <RiFileLine className="h-3.5 w-3.5 text-cyan-500" />;
      case 'research':
      case 'analyze':
      case 'review':
      case 'risk':
      case 'privilege':
        return <RiFlashlightLine className="h-3.5 w-3.5 text-cyan-500" />;
      default:
