...
If no mode is specified, you MUST use the **question** tool to present an interactive selection menu. Call the question tool with:

```json
{
  "questions": [
    {
      "question": "What type of legal briefing do you need?",
      "header": "Brief Type",
      "options": [
        {"label": "Daily Brief", "description": "Morning summary of legal-relevant items from email, calendar, chat, CLM, and CRM"},
        {"label": "Topic Brief", "description": "Research brief on a specific legal question"},
        {"label": "Incident Brief", "description": "Rapid briefing for a developing situation requiring immediate legal attention"}
      ]
    }
  ]
}
...
- For **Daily Brief**: proceed directly to generating the brief
- For **Topic Brief**: use the question tool again to ask "What legal topic or question would you like me to research?" then proceed
- For **Incident Brief**: use the question tool again to ask "What is the incident or situation that needs immediate attention?" then proceed
...
- **File format handling**: If the user references documents (PDFs, DOCX) for context, use the available file-reading tools to access them:
  1. Use Glob to search for the file if only a filename is given (search `~/Documents/`, `~/Downloads/`, `~/Desktop/`, and the current workspace)
  2. Use the Read tool to read the file contents — the Read tool supports PDFs, images, and text files natively
  3. If the file is a URL, use WebFetch to retrieve it
  4. Only ask the user to paste text as a last resort if all tool-based approaches fail
