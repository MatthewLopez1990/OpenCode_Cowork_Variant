...
- **Reading files from the filesystem**: If the user provides a filename or path to a document (PDF, DOCX, etc.), use Glob to search for it in common locations (`~/Documents/**`, `~/Downloads/**`, `~/Desktop/**`, and the workspace) and then use the Read tool to read it — the Read tool supports PDFs natively regardless of model provider
